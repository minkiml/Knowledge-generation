...
class MLPMixer(nn.Module):
    def __init__(self, input_dim = None, image_size = None, num_classes = None, num_layers = None):
        super(MLPMixer, self).__init__()

        # Arguments
        in_channels=mlpmixer_args["in_channels"] if input_dim is None else input_dim
        img_size=mlpmixer_args["img_size"] if image_size is None else image_size
        patch_size=mlpmixer_args["patch_size"]
        hidden_size=mlpmixer_args["hidden_size"]
        hidden_s=mlpmixer_args["hidden_s"]
        hidden_c=mlpmixer_args["hidden_c"]
        num_layers=mlpmixer_args["num_layers"] if num_layers is None else num_layers
        num_classes=mlpmixer_args["num_classes"] if num_classes is None else num_classes
        drop_p=mlpmixer_args["drop_p"]
        off_act=mlpmixer_args["off_act"]
        is_cls_token=mlpmixer_args["is_cls_token"]
