'''
Non-differentiable AM permutation search algorithms
'''
...
def match_tensors_zipit(
    ...
    **kwargs
    ...
    #     # print("cov",metric["covariance"].shape)
    #     sims = compute_correlation(metric)
    # elif "cossim" in metric:
    #     sims = metric["cossim"]
    sims = metric
    # print(sims.shape)
    O = sims.shape[0] # concated shaped
    remainder = int(O * (1-r) + 1e-4)
    permutation_matrix = torch.eye(O, O)#, device=sims.device)

    torch.diagonal(sims)[:] = -torch.inf  # make the diagonal part -inf

    num_models = int(1/(1 - r) + 0.5)
    Om = O // num_models # original model shape
...
def match_tensors_permute(metric, r=.5, get_merge_value=False, add_bias=False, **kwargs):
    ...
    """
    correlation = metric # compute_correlation(metric["covariance"])
...
def match_tensors_cca(metric, reduce_ratio=.5, get_merge_value=False, add_bias=False, gamma=0.1, 
                      dual = False, **kwargs):
    """
    Matches arbitrary models by CCA. Transforms all models to the space of the first one.
    """
    covariance = metric # metric["covariance"]
    ...
    if dual:
        ...
        unmerge_mats_2 = [torch.eye(Om, device=device)]
    for i in range(1, num_models):
        try:
            ...
            if dual:
                y_map_wrt_A = S_xx_neg_sqrt @ U @ V.T @ S_yy_sqrt.T # S_yy_neg_sqrt @ V @ U.T @ S_xx_sqrt.T
                y_unmerge_map_wrt_A = S_yy_neg_sqrt @ V @ U.T @ S_xx_sqrt.T# S_xx_neg_sqrt @ U @ V.T @ S_yy_sqrt.T
        ...
        if dual:
            mats_2.insert(0, y_map_wrt_A)
            unmerge_mats_2.insert(0, y_unmerge_map_wrt_A.T)

    unmerge = torch.cat(unmerge_mats, dim=0)
    merge = torch.cat(mats, dim=0)
    merge = merge #/ num_models
    if dual:
        unmerge2 = torch.cat(unmerge_mats_2, dim=0)
        merge2 = torch.cat(mats_2, dim=0)
        merge2 = merge2 #/ num_models  

        return [merge.T, unmerge], [merge2.T, unmerge2]
    else:
        return merge.T, unmerge
...
def match_tensors_optimal(metric, r=.5, add_bias=False, **kwargs):
    ...
    Hyperparameters and return are as defined in match_tensors_zipit.
    """
    correlation = metric # metric["covariance"]
    corr_mtx_a = correlation.cpu().numpy() # compute_correlation(correlation).cpu().numpy()
...
def get_head_perm(inner_perm, inner_perm_inv, head_perm):
    ''' 
    Get outer permutation given head_perm &
    Apply the head perm to inner_perm and inner_perm_inv, then reshape them into (num_heads, d/h, d/h)
    '''
    head_num = len(head_perm)
    head_dim = inner_perm.shape[0] // head_num
    inner_perms = []
    inner_perms_inv = []

    for col in range(head_num):
        row = np.where(head_perm == col)[0][0] # .index(col)
        inner_perms.append(inner_perm[row*head_dim:row*head_dim+head_dim, col*head_dim:col*head_dim+head_dim].unsqueeze(0))
        inner_perms_inv.append(inner_perm_inv[row*head_dim:row*head_dim+head_dim, col*head_dim:col*head_dim+head_dim].unsqueeze(0))
    # Note that this multi_head_output perm is already in right order by accounting for the head (outer) permutation
    inner_perms = torch.cat((inner_perms), dim = 0)
    inner_perms_inv = torch.cat((inner_perms_inv), dim = 0)

    head_perm = torch.eye(head_num)[head_perm]
    head_perm_inv = head_perm.T

    return inner_perms, inner_perms_inv, head_perm, head_perm_inv # (num_heads, d/h, d/h), (head, head)

def match_tensors_permute_MHA(n_heads, permute_heads=False, 
                              head_assignments=[], r=.5,
                              no_absval=False, 
                              correlation_matrix=None):
    """
    Handles different head permutations in attention
    Source: https://github.com/nverma1/merging-text-transformers/blob/main/matching_functions.py
    Paper: Merging Text Transformer Models from Different Initializations

    Original attention - (attention{ (XQ) (XK)^T } * (XV)) * W_p
        
        Consider below, 
        
        1. multi-head permutation: Two possible permutations; 1) inner permutation (within each head) and 2) outer permutation (between heads) 
        
        2. V-W_p permutation
        
        3. Q-K permutation --> Find single joint P such that XQ_1 = XQ_2P  and (XK_1)^T = (XK_2P)^T - (optionally activated with QK_permutation = True and inner_permutation = True)

        Note that inner permuations of {Query, Key} are inpendent from the inner permutations of {Value and W_proj}, 
        but the same outer permutation must be applied to all {Q, K, V, W_proj}  
    ...
    new_mat = torch.eye(Om, device=device)[torch.cat(head_perms).clone().detach().long().to(device)]
    ...
    return merge.T, unmerge, outer_col_ind
